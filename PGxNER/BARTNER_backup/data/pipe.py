...
class DiscontinuousNERLoader(Loader):
    ...
    def _load(self, path):
        """
        entities: List[List[str]], each element is an entity, non-contiguous spelled together
        entity_tags: same length as above, is the fraction of each tag
        raw_words: List[str] words
        entity_spans: List[List[int]] records the start and end of the above entity, 
        here the length must be even, is the pair of start,end
        ...
        for i in range(len(lines)):
            if i % 3 == 0:
                ...
                ann = lines[i + 1]
                now_ins = Instance()
                sentence = sentence.strip().split(' ')  # Generated spaces
                ...
                for entity in entities:
                    ...
                    type_list.append(type_.lower())  # Internal is str
                    entity_list.append(str_span__)
                    entity_index_list.append(list(chain(*span__)))  # Internal is integer
                ...
                now_ins.add_field('raw_words', sentence)  # Words separated by spaces
