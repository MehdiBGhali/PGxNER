...
"""elif dataset_name == 'genia':
    ...
    print(data_bundle)
    callbacks.append(CustomFitlogCallback(data_bundle.get_dataset('test'), raise_threshold=0.04, history_dir = history_dir, eval_begin_epoch=eval_start_epoch, summary=args))  # 如果低于0.04大概率是讯飞了
    fitlog.add_other(name='demo', value='split dev')
else:
    callbacks.append(CustomFitlogCallback(raise_threshold=0.04, history_dir = history_dir, eval_begin_epoch=eval_start_epoch, summary=args))  # 如果低于0.04大概率是讯飞了
