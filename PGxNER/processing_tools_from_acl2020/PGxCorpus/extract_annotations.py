...
def parse_parameters(parser=None):
    ...
    ## Required
    parser.add_argument("--input_text", default="/PGxNER/raw_data/PGxCorpus/text", type=str)
    parser.add_argument("--input_ann", default="/PGxNER/raw_data/PGxCorpus/ann", type=str)
    parser.add_argument("--output_filepath", default="/PGxNER/Data_Processing/PGxCorpus/Storage", type=str)
    ...
    with open(args.output_filepath, "w") as out_f:
        for document in os.listdir(args.input_ann):
            with open(os.path.join(args.input_ann, document), "r") as in_f:
                ...
                for line in in_f:
                    line = line.strip()
                    if line[0] != "T": continue # This ignores the "R" relationship type annotations, which we do not need anyways
                    sp = line.strip().split("\t")
                    assert len(sp) == 3
                    mention = sp[2]
                    sp = sp[1].split(" ")
